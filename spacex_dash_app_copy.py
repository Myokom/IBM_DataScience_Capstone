...
options = [{'label': 'All Sites', 'value': 'ALL'}] + [{'label': site, 'value': site} for site in unique_launch_sites]
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        # Create a pie chart with filtered data
        fig = px.pie(
            filtered_df,
            names='class',
...
def update_scatter_plot(selected_site, payload_range):
    # Filter based on the selected site if not 'ALL'
    ...
    fig = px.scatter(
        ...
        title='Correlation between Payload and Success for each Booster Version'
    )
    return fig
